...
def main(argv):
    ...
    print("This is an interactive tool to check the status of the current jobs.\n"
          "Type \"active_jobs\" to see what is still running. To kill a child thread,\n"
          "use command: \"kill #\", where # "
          "represents the thread number (from active_jobs).\n"
          "Use command \"convert_to_csv 'name'\" to convert any "
          "'name'.json file to 'name'.csv.\nFinally, type \"exit\" to end the program. "
          "Check the log files for any warnings or errors.")
    command = ""
    while command != "exit":
        command = input("> ")
